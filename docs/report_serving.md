# 기능요구사항 6~8항: 모델 서빙, 대시보드, MLOps 환경 보고서
...
4. [개발 환경 및 MLOps](#4-개발-환경-및-mlops)
5. [유닛 테스트](#5-유닛-테스트)
...
Streamlit을 사용하여 판독 보조 대시보드를 구현하였다. 대시보드는 모델(.pth)을 직접 로드하지 않고, 반드시 FastAPI 서버와 HTTP 통신(`requests.post`)하여 추론을 수행한다. 제출용 Docker 이미지는 GPU 없이 CPU 기준으로 동작하며, 순수 추론은 ONNX Runtime `CPUExecutionProvider`에서 수행된다.
...
| 항목 | 값 |
|------|---|
| 기준 장비 | Apple M1 Pro MacBook Pro (`MacBookPro18,3`) |
| 메모리 | Host RAM 32GB, Docker Desktop 할당 메모리 약 8GB |
| 환경 | Docker 컨테이너 (python:3.12-slim, linux/aarch64) |
| CPU | Docker Desktop 할당 CPU 10 cores |
| 추론 엔진 | ONNX Runtime 1.20+ (CPUExecutionProvider) |
| 스레드 | intra_op=4, inter_op=1 (모델별) |
| 측정 방식 | 워밍업 후 5회 평균 |

본 벤치마크는 CPU 기반 서빙 성능을 측정한 결과이다. FastAPI/Streamlit 대시보드 실행에는 CUDA 또는 GPU가 필요하지 않으며, 다른 CPU 성능이나 Docker 리소스 할당량에 따라 지연시간은 달라질 수 있다.
...
본 프로젝트의 MLOps 환경은 모델을 재현 가능하게 실행하고 검증하기 위한 최소 운영 단위로 정의하였다. Docker 기반 실행 환경, 모델/설정 파일 분리, API health check, Streamlit-API 역할 분리, pytest 검증, Docker Hub 이미지 배포를 포함한다.
...
| 구성 요소 | 구현 내용 |
|-----------|----------|
| 실행 환경 표준화 | `Dockerfile`, `requirements.txt`, `start.sh`로 FastAPI와 Streamlit을 동일 컨테이너에서 실행 |
| 이미지 배포 | Docker Hub `mougam/cxr-cad-final` 이미지 제공 |
| Health Check | `/health` 엔드포인트로 서버 상태와 모델 로드 여부 확인 |
| 역할 분리 | Streamlit은 모델을 직접 로드하지 않고 FastAPI와 HTTP 통신 |
| 설정 관리 | `app/config.py`, `model_assets/*.json`으로 모델 경로, threshold, calibration 파라미터 관리 |
| 모델 산출물 관리 | `.pth`, `.onnx`, threshold, Platt scaling JSON을 모델별 디렉토리로 분리 |
| 테스트 | `pytest` 기반 전처리, label, API schema, 모델 구조 테스트 32개 통과 |
