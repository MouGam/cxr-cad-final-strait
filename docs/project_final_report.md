...
프로젝트 초기에는 3인 팀을 전제로 전처리, 학습, 서빙, 문서화 업무를 나누는 방향을 구상하였다. 그러나 한 명이 피치 못할 사정으로 이탈하면서 실제 수행 인원이 2명으로 줄었고, 기존에 구상했던 업무분담과 실제 수행 방식 사이에 차이가 생겼다. 이에 따라 PM은 남은 요구사항과 산출물 범위를 다시 정리하고, 2인 체제에서 완수 가능한 우선순위와 역할 조정 방식을 재설계해야 했다.

업무분장표는 2인 체제로 전환된 뒤 작성하였으나, 실제 수행 과정에서는 업무가 표처럼 기계적으로 나뉘지는 않았다. 전처리, 학습, 평가, 서빙, 대시보드, 문서화가 서로 맞물려 있었기 때문에 진행 상황에 따라 필요한 작업을 유연하게 가져가고, 막히는 영역은 회의에서 즉시 공유하여 조정하는 방식으로 운영하였다. 주 1회 대면 회의를 기준으로 진행 범위, 남은 작업, 주간 할당량, 문제 상황을 점검하였고, 전처리/학습 repo와 서빙 showcase repo에 나뉘어 있던 결과물을 최종 제출 repo로 통합하였다.

2인 체제는 각 팀원의 부담을 키웠지만, 동시에 의사소통과 의사결정 속도를 높이는 효과도 있었다. 논의 대상과 결정자가 두 명으로 줄어들면서 방향 전환이 빠르게 이루어졌고, 요구사항을 충족하기 위해 필요한 작업을 즉시 재배치할 수 있었다.
...
프로젝트는 최종적으로 2인 팀으로 수행되었으며, 역할은 프로젝트 관리/전처리/프론트엔드와 모델 학습/백엔드/Docker를 중심으로 나누어 진행하였다. 초기에는 3인 수행을 전제로 일부 업무를 구상했기 때문에 2인 전환 후에는 각자 맡아야 할 범위가 넓어졌고, 역할 경계를 고정하기보다 산출물 완성을 기준으로 조정하는 방식이 필요했다. 개인별 정성 평가는 본 보고서에서 다루지 않고, 역할 분담과 산출물 기반으로 수행 결과를 정리한다.
...
인력 이용 측면의 가장 큰 문제는 3인 기준으로 구상했던 업무 범위를 2인 체제로 완수해야 했다는 점이다. 전처리, 모델 학습, 평가, 서빙, 대시보드, 문서화까지 모두 포함된 과제였기 때문에 한 사람이 한 영역만 고정적으로 수행하는 방식으로는 병목이 생길 수 있었다.

이를 해결하기 위해 PM은 전체 요구사항과 제출 산출물을 기준으로 우선순위를 관리하고, 진행 상황에 따라 업무를 유동적으로 조정하였다. 전민혁은 프로젝트 관리, 전처리, 프론트엔드, 문서 통합 등 전체 구조를 보완하는 역할을 수행하였고, 김찬영 팀원은 부담이 커진 상황에서도 모델 학습, 백엔드, Docker, 발표 준비 등 핵심 영역을 끝까지 수행하여 최종 산출물 완성에 기여하였다. 최종 단계에서는 repo 통합과 문서 정리의 비중이 커졌으며, 기능별 리포트를 분리하여 관리함으로써 검토 범위를 줄였다.

결과적으로 2인 체제는 업무 부담을 증가시켰지만, 빠른 의사소통과 유연한 의사결정이라는 장점도 만들었다. 문제 상황이 생기면 두 사람이 바로 논의하여 방향을 정할 수 있었고, 업무분장표의 경계를 엄격히 고수하기보다 최종 제출물 완성에 필요한 방향으로 작업을 재분배할 수 있었다.
