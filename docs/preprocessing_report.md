...
1. [데이터셋 개요](#1-데이터셋-개요)
2. [DICOM 파싱 및 PHI 처리](#2-dicom-파싱-및-phi-처리)
3. [이미지 전처리](#3-이미지-전처리)
4. [이미지 품질 필터링](#4-이미지-품질-필터링)
5. [Multi-hot Encoding](#5-multi-hot-encoding)
6. [데이터 분할](#6-데이터-분할)
...
총 **141장 제거**, **31장 수동 처리 후 포함**(30장 crop, 1장 CLAHE-only), 최종 **111,979장** 사용.
...
**의사결정:** 소아 희귀 질환 28장 + 밝은 이미지 2장은 수동 crop 후 포함하고, 저대비 1장은 crop 없이 CLAHE 재처리 대상으로 포함하기로 결정하였다. 따라서 총 **31장**을 수동 처리 후 available 데이터셋에 포함하였다.
...
의료 데이터의 특성상 완전 자동화보다 **"자동 crop + 수동 검증"** 방식을 채택하여 신뢰성을 확보하였다. 실제 crop 대상은 30장이며, 저대비 1장은 crop 없이 CLAHE 재처리로 포함하였다. 소규모 대상이므로 수동 검수 비용이 낮았다.
